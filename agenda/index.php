...
$result = $stmt->get_result(); // get the mysqli result
  ...
  <A HREF="#" onMouseOver="window.status='Click for popup day schedule';return true" onMouseOut="window.status='';return true" onClick="popup('agenda/add.php?daystamp=<?php echo $string1?>', 'Win1', 600, 275); return false" class="daylink">
   ...
   if(strftime("%Y-%m-%d",$rs['datestamp'])== $string1)
   {
			...
			else
			{
				...
				?>
				<span title="<?php echo "$desc  ($loc)";?>"><li>
				<A HREF="#"  alt="<?php echo $desc.'-'.$id_progetto.'-'.$id_corso?>" onMouseOver="window.status='Click for popup detail';return true" onMouseOut="window.status='';return true" onClick="popup('agenda/popdetail.php?loc=<?php echo $loc?>&nome=<?php echo $id_nome?>&cognome=<?php echo $id_cognome?>&stime=<?php echo $stime?>&etime=<?php echo $etime?>&desc=<?php echo $desc?>&id=<?php echo $thisid?>&daystamp=<?php echo $daystamp?>', 'Win1', 600, 125); return false" class="eventlink" style="color:<?php echo $colortesto?>">
				<?php echo $stime."-".$etime."&nbsp".$loc.";Progetto:".$id_progetto.";Corso:".$id_corso.";Modulo:".$id_modulo.";Docente:".$id_nome." ".$id_cognome."<BR>";?>
				</a></li></span>
				<?php 
			}
